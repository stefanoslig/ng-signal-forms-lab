...
Based on the logic we have defined using the `Schema` and the state derived from user actions (`touched`), we get the `FieldState` for every node in the `FieldTree`. To retrieve the `FieldState`, navigate to a node in the `FieldTree` and invoke it as a function.
